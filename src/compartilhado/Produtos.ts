import bifinhoChurrasco from '../assets/images/produtos/bifinhoChurrasco.webp'
import bifinhoCarne from '../assets/images/produtos/bifinhoCarne.webp'
import bifinhoGalinha from '../assets/images/produtos/bifinhoGalinha.webp'
import zoolitos from '../assets/images/produtos/zoolitos.webp'
import bifinhoPeru from '../assets/images/produtos/bifinhoPeru.webp'
import steakFrango from '../assets/images/produtos/steakFrango.webp'
import orelhaBovina from '../assets/images/produtos/orelhaBovina.webp'
import orelhaSuina from '../assets/images/produtos/orelhaSuina.webp'
import femurBovino from '../assets/images/produtos/femurBovino.webp'
import femurSuino from '../assets/images/produtos/femurSuino.webp'
import mixRecortes from '../assets/images/produtos/mixRecortes.webp'
import ossoCabeca from '../assets/images/produtos/ossoCabeca.webp'
import costela from '../assets/images/produtos/costela.webp'

export const description = [
    {
        descrition1: 'No mundo dos animais de estimação, alimentação é coisa séria e é por isso que a zoomies fez questão de criar petiscos que são um verdadeiro espetáculo - deliciosos e cheios de ingredientes saudáveis e nutritivos.'
    },

    {
        descrition2:'As nossas embalagens super estilosas e coloridas estão pedindo para serem escolhidas pelos donos mais amorosos e preocupados, que sabem que os pets merecem só o melhor.'
    },
    
    {
        descrition3: 'E se você acha que paramos por aí, está muito enganado a zoomies é o caminho certo para os pais de pet que amam ver seus filhos peludos felizes, saudáveis e cheios de energia.'
    },
    
    {
        descrition4: 'Vem com a gente na zoomies, onde diversão e saúde são os nossos ingredientes secretos!'
    }
]

export const bifinhos = [
    {
        nome: 'Bifinho Churrasco',
        imagem: bifinhoChurrasco,
        descricao: 'Transformamos o aroma inconfundível do churrasco em um \
            formato perfeito para compartilhar com seu melhor amigo peludo. \
            Sabemos que as tardes de churrasco são sagradas para a família \
            brasileira e agora, com o novo sabor, você pode incluir seu \
            cãozinho nesse momento de união e alegria.',
        composicao: 'Pulmão suíno, carne mecanicamente separada de aves, carne de bovino, proteína texturizada de soja*, glicerina purificada, sal refinado, aroma idêntico ao natural de churrasco, hidrolisado de fígado de suíno, propilgalato, antioxidantes (BHT e BHA), sorbato de potássio, goma xantana, ácido fosfórico,  ácido cítrico, ácido sórbico, tripolifosfato de sódio, dióxido de silício,  ácido propiônico, amônia, aroma cítrico, benzoato de sódio, ácido tartárico e corante caramelo IV.',
        doadores: 'Agrobacterium tumefaciens, Bacillus thuringiensis e Streptomyces viridochromogenes.',
        substitutos: 'Pulmão bovino e aroma idêntico ao natural de carne e aroma idêntico ao natural de picanha.',
        garantias: 'Umidade (máx.) 250 g/kg (25%), proteína bruta (mín.) 300 g/kg (30%), extrato etéreo (mín.) 70 g/kg (7%), matéria fibrosa (máx.) 20 g/kg (2%), matéria mineral (máx.) 100 g/kg (10%), cálcio (máx.) 20 g/kg (2%), cálcio (mín.) 3000 mg/kg (0,3%), fósforo (mín.) 4000 mg/kg (0,4%).',
        linha: 'bifinhos',
    },
    
    {
        nome: 'Bifinho Carne para filhotes',
        imagem: bifinhoCarne,
        descricao: 'Desenvolvemos uma linha de bifinhos super especial para os \
            filhotinhos de 4 patas, com nutrientes específicos para que \
            cresçam de forma saudável. Cuidadosamente, escolhemos os \
            sabores “carne” e “peito de peru”, para criar uma experiência \
            deliciosa e nutritiva para os filhotes.',
        composicao: 'Pulmão suíno, carne mecanicamente separada de aves, carne de bovino, proteína texturizada de soja, glicerina purificada, sal refinado, aroma artificial de carne defumada, carbonato de cálcio, hidrolisado de fígado de suíno, propilgalato, antioxidantes (BHT e BHA), sorbato de potássio, goma xantana, ácido fosfórico,  ácido cítrico, ácido sórbico, tripolifosfato de sódio, dióxido de silício,  ácido propiônico, amônia, aroma cítrico, benzoato de sódio, ácido tartárico e corante caramelo IV.',
        doadores: ' Agrobacterium tumefaciens, Bacillus thuringiensis e Streptomyces viridochromogenes. Eventuais Substitutos: Pulmão bovino e aroma idêntico ao natural de carne.',
        garantias: 'Umidade (máx.) 250 g/kg (25%), proteína bruta (mín.) 300 g/kg (30%), extrato etéreo (mín.) 70 g/kg (7%), matéria fibrosa (máx.) 20 g/kg (2%), matéria mineral (máx.) 100 g/kg (10%), cálcio (máx.) 40 g/kg (4%), cálcio (mín.) 3000 mg/kg (0,3%), fósforo (mín.) 4000 mg/kg (0,4%).',
        linha: 'bifinhos',
    },

    {
        nome: 'Bifinho Peito de Peru para filhotes',
        imagem: bifinhoPeru,
        descricao: 'Desenvolvemos uma linha de bifinhos super especial para os \
            filhotinhos de 4 patas, com nutrientes específicos para que \
            cresçam de forma saudável. Cuidadosamente, escolhemos os \
            sabores “carne” e “peito de peru”, para criar uma experiência \
            deliciosa e nutritiva para os filhotes.',
        composicao: 'Pulmão suíno, carne mecanicamente separada de aves, carne de bovino, proteína texturizada de soja, glicerina purificada, sal refinado, aroma natural de peito de peru, carbonato de cálcio, hidrolisado de fígado de suíno, propilgalato, antioxidantes (BHT e BHA), sorbato de potássio, goma xantana, ácido fosfórico,  ácido cítrico, ácido sórbico, tripolifosfato de sódio, dióxido de silício,  ácido propiônico, amônia, aroma cítrico, benzoato de sódio e ácido tartárico.',
        doadores: 'Agrobacterium tumefaciens, Bacillus thuringiensis e Streptomyces viridochromogenes.',
        substitutos: 'Pulmão bovino e sorbato de potássio.',
        garantias: 'Umidade (máx.) 250 g/kg (25%), proteína bruta (mín.) 350 g/kg (35%), extrato etéreo (mín.) 70 g/kg (7%), matéria fibrosa (máx.) 20 g/kg (2%), matéria mineral (máx.) 100 g/kg (10%), cálcio (máx.) 40 g/kg (4%), cálcio (mín.) 3000 mg/kg (0,3%), fósforo (mín.) 4000 mg/kg (0,4%).',
        linha: 'bifinhos',
    },

    {
        nome: 'Bifinho Zoolitos Calabresa',
        imagem: zoolitos,
        descricao: "Com Zoolitos, você pode premiar o seu cãozinho em diferentes \
            momentos, como quando ele se mostra obediente durante os \
            treinamentos ou quando você quer oferecer aquele mimo especial. \
            Esse petisco é compatível com todas as idades e portes de \
            cães e, se quiser oferecer para filhotes ou cães de porte pequeno, \
            é só fracionar o bifinho. Dá pra todo  mundo aproveitar!",
        composicao: 'Pulmão suíno, carne mecanicamente separada de aves, carne de bovino, proteína texturizada de soja*, glicerina purificada, sal refinado, aroma idêntico ao natural de calabresa, hidrolisado de fígado de suíno, propilgalato, antioxidantes (BHT e BHA), sorbato de potássio, goma xantana, ácido fosfórico,  ácido cítrico, ácido sórbico, tripolifosfato de sódio, dióxido de silício,  ácido propiônico, amônia, aroma cítrico, benzoato de sódio, ácido tartárico e corante vermelho allura.',
        doadores: 'Agrobacterium tumefaciens, Bacillus thuringiensis e Streptomyces viridochromogenes.',
        substitutos: 'Pulmão bovino.',
        garantias: 'Umidade (máx.) 250 g/kg (25%), proteína bruta (mín.) 300 g/kg (30%), extrato etéreo (mín.) 70 g/kg (7%), matéria fibrosa (máx.) 20 g/kg (2%), matéria mineral (máx.) 100 g/kg (10%), cálcio (máx.) 20 g/kg (2%), cálcio (mín.) 3000 mg/kg (0,3%), fósforo (mín.) 4000 mg/kg (0,4%).',
        linha: 'bifinhos',
    },

    {
        nome: 'Bifinho Galinha Caipira',
        imagem: bifinhoGalinha,
        descricao: 'Com este sabor, tentamos capturar a essência da culinária familiar, \
            aquela que nos lembra os dias de infância quando a vovó \
            cozinhava os seus melhores pratos. Seja em uma reconfortante \
            canja em dias frios ou em um frango ensopado tão delicioso, \
            nosso objetivo é trazer esta tradição culinária, rica em temperos \
            típicos do Brasil, para o universo dos animais de estimação.',
        composicao: 'Pulmão suíno, carne mecanicamente separada de aves, carne de bovino,  proteína texturizada de soja*, glicerina purificada, sal refinado, aroma artificial de galinha, hidrolisado de fígado de suíno, propilgalato, antioxidantes (BHT e BHA), sorbato de potássio, goma xantana, ácido fosfórico,  ácido cítrico, ácido sórbico, tripolifosfato de sódio, dióxido de silício,  ácido propiônico, amônia, aroma cítrico, benzoato de sódio e ácido tartárico.',
        doadores: ' Agrobacterium tumefaciens, Bacillus thuringiensis e Streptomyces viridochromogenes.',
        substitutos: 'Pulmão bovino e aroma idêntico ao natural de frango.',
        garantias: 'Umidade (máx.) 250 g/kg (25%), proteína bruta (mín.) 350 g/kg (35%), extrato etéreo (mín.) 70 g/kg (7%), matéria fibrosa (máx.) 20 g/kg (2%), matéria mineral (máx.) 100 g/kg (10%), cálcio (máx.) 20 g/kg (2%), cálcio (mín.) 3000 mg/kg (0,3%), fósforo (mín.) 4000 mg/kg (0,4%).',
        linha: 'bifinhos',
    },
]

export const naturais = [
    {
        nome: 'Steak de Frango',
        imagem: steakFrango,
        descricao: 'Steaks de Frango Zoomies podem ser oferecidos aos cães de todos os portes como uma recompensa em treinamentos, ao se mostrar obediente e como um mimo. Este alimento pode ser oferecido a todas as idades e portes de cães, quando oferecido para cães de porte pequeno ou filhotes, recomenda-se fracionar o steak.',
        composicao: 'Carne de aves, glicerina, água, cloreto de sódio e sorbato de potássio.',
        linha: 'naturais',
    },

    {
        nome: 'Osso de Costela Bovina',
        imagem: costela,
        descricao: 'O estresse pode causar diversos problemas de saúde nos cães, por isso criamos uma \
            linha de produtos especiais que ajudam os animais a controlar a sua ansiedade e irritação. \
            Essa linha de naturais antiestresse veio para ajudar o seu cãozinho a ter mais tranquilidade no dia \
            a dia, unindo bem-estar e lazer, fazendo com que ele se sinta verdadeiramente feliz e saudável.',
        composicao: 'Osso bovino de costela desidratado. Este produto não substitui um alimento completo. Uso proibido na alimentação de ruminantes. Distribuir conforme indicação de uso, como forma de premiação, distração e brincadeira.',
        linha: 'naturais',
    },

    {
        nome: 'Osso de Fêmur Bovino',
        imagem: femurBovino,
        descricao: 'O estresse pode causar diversos problemas de saúde nos cães, por isso criamos uma \
            linha de produtos especiais que ajudam os animais a controlar a sua ansiedade e irritação. \
            Essa linha de naturais antiestresse veio para ajudar o seu cãozinho a ter mais tranquilidade no dia \
            a dia, unindo bem-estar e lazer, fazendo com que ele se sinta verdadeiramente feliz e saudável.',
        composicao: 'Osso desidratado bovino. Este produto não substitui um alimento completo. Uso proibido na alimentação de ruminantes. Distribuir conforme indicação de uso, como forma de premiação, distração e brincadeira.',
        linha: 'naturais',
    },

    {
        nome: 'Osso de Fêmur Suíno',
        imagem: femurSuino,
        descricao: 'O estresse pode causar diversos problemas de saúde nos cães, por isso criamos uma \
            linha de produtos especiais que ajudam os animais a controlar a sua ansiedade e irritação. \
            Essa linha de naturais antiestresse veio para ajudar o seu cãozinho a ter mais tranquilidade no dia \
            a dia, unindo bem-estar e lazer, fazendo com que ele se sinta verdadeiramente feliz e saudável.',
        composicao: 'Osso desidratado suíno. Este produto não substitui um alimento completo. Uso proibido na alimentação de ruminantes. Distribuir conforme indicação de uso, como forma de premiação, distração e brincadeira.',
        linha: 'naturais',
    },

    {
        nome: 'Cabeça de Fêmur',
        imagem: ossoCabeca,
        descricao: 'O estresse pode causar diversos problemas de saúde nos cães, por isso criamos uma \
            linha de produtos especiais que ajudam os animais a controlar a sua ansiedade e irritação. \
            Essa linha de naturais antiestresse veio para ajudar o seu cãozinho a ter mais tranquilidade no dia \
            a dia, unindo bem-estar e lazer, fazendo com que ele se sinta verdadeiramente feliz e saudável.',
        composicao: 'Osso desidratado bovino. Este produto não substitui um alimento completo. Uso proibido na alimentação de ruminantes. Distribuir conforme indicação de uso, como forma de premiação, distração e brincadeira.',
        linha: 'naturais',
    },

    {
        nome: 'Orelha Bovina',
        imagem: orelhaBovina,
        descricao: 'O estresse pode causar diversos problemas de saúde nos cães, por isso criamos uma \
            linha de produtos especiais que ajudam os animais a controlar a sua ansiedade e irritação. \
            Essa linha de naturais antiestresse veio para ajudar o seu cãozinho a ter mais tranquilidade no dia \
            a dia, unindo bem-estar e lazer, fazendo com que ele se sinta verdadeiramente feliz e saudável.',
        composicao: 'Orelha bovina desidratada. Este produto não substitui um alimento completo. Uso proibido na alimentação de ruminantes. Distribuir conforme indicação de uso, como forma de premiação, distração e brincadeira.',
        linha: 'naturais',
    },

    {
        nome: 'Orelha Suína',
        imagem: orelhaSuina,
        descricao: 'O estresse pode causar diversos problemas de saúde nos cães, por isso criamos uma \
            linha de produtos especiais que ajudam os animais a controlar a sua ansiedade e irritação. \
            Essa linha de naturais antiestresse veio para ajudar o seu cãozinho a ter mais tranquilidade no dia \
            a dia, unindo bem-estar e lazer, fazendo com que ele se sinta verdadeiramente feliz e saudável.',
        composicao: 'Orelha suína desidratada. Este produto não substitui um alimento completo. Uso proibido na alimentação de ruminantes. Distribuir conforme indicação de uso, como forma de premiação, distração e brincadeira.',
        linha: 'naturais',
    },
    
    {
        nome: 'Mix de Recorte',
        imagem: mixRecortes,
        descricao: 'O estresse pode causar diversos problemas de saúde nos cães, por isso criamos uma \
            linha de produtos especiais que ajudam os animais a controlar a sua ansiedade e irritação. \
            Essa linha de naturais antiestresse veio para ajudar o seu cãozinho a ter mais tranquilidade no dia \
            a dia, unindo bem-estar e lazer, fazendo com que ele se sinta verdadeiramente feliz e saudável.',
        composicao: 'Recortes desidratados de bovinos. No mix pode conter recortes de traqueia, esôfago e vergalho. Este produto não substitui um alimento completo. Uso proibido na alimentação de ruminantes. Distribuir conforme indicação de uso, como forma de premiação, distração e brincadeira.',
        linha: 'naturais',
    },
]

export const AllProducts = [
    ...bifinhos,
    ...naturais,
]